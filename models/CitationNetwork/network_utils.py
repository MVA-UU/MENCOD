...
def _load_synergy_dataset(dataset_name: str) -> Optional[Dict]:
    ...
    try:
        ...
        config_path = os.path.join(os.path.dirname(os.path.dirname(current_dir)), 'data', 'datasets.json')
...
def add_semantic_edges(G: nx.DiGraph, simulation_df: pd.DataFrame) -> None:
    """Add enhanced semantic similarity edges that prioritize outlier detection patterns."""
